...
def train_dino(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationDINO4K(args.local_crops_number)
...
### Custom Dataset Implemented to Load in [16 x 16 x 384-Dim] Tensors which correspond to extracted ViT-16 features for 4K x 4K patch
class SeqDataset(Dataset):
    def __init__(self, dataroot, transform):
        self.seq_list = list(Path(dataroot).glob("*/l*.npy"))
        self.transform = transform

    def __getitem__(self, index):
        seq = torch.from_numpy(np.load(self.seq_list[index])).swapaxes(2, 0)
...
if __name__ == "__main__":
    ...
    args.data_path = "/data/datasets/PANCAN/pathology_tiles/parafine/COAD/HIPT/patch_3584_pretraining/"
    args.output_dir = (
        "/data/datasets/PANCAN/pathology_tiles/parafine/COAD/HIPT/dino_256/"
    )
