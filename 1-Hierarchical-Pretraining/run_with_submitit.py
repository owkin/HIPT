...
def parse_args():
    ...
    parser.add_argument(
        "--partition", default="gpu_p13", type=str, help="Partition where to submit"
    )
    parser.add_argument(
        "--constraint", default=1200, type=int, help="Duration of the job (min)"
    )
    parser.add_argument(
        "--comment",
        default="",
...
#     user = os.getenv("USER")
#     if Path("/gpfsscratch/rech/htc/uxu85mw/ibot/checkpoint/").is_dir():
#         p = Path(f"/gpfsscratch/rech/htc/uxu85mw/ibot/checkpoint/{user}/experiments")
...
def get_shared_folder() -> Path:
    user = os.getenv("USER")
    if Path("/checkpoint/").is_dir():  # TODO: change
